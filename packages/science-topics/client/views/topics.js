Template.Topics.onRendered(function () {
    ...
    $('#tree').treeview({levels: 1, showBorder: false, data: flatTopicsToTreeNodes(isLangCn)});
})
flatTopicsToTreeNodes = function(isLangCn){
    var topicTree = [];
    _.each(Topics.find({parentId: null}).fetch(), function (topic) {
        var thisTopic = {text: isLangCn ? topic.name : topic.englishName};
        var childTopics = [];
        _.each(Topics.find({parentId: topic._id}).fetch(), function (child) {
            var oneChild = {text: isLangCn ? child.name : child.englishName};
            var grandchildTopics = [];
            _.each(Topics.find({parentId: child._id}).fetch(), function (grandchild) {
                var oneGrandchild = {text: isLangCn ? grandchild.name : grandchild.englishName};
