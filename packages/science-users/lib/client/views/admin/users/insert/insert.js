...
Template.AdminUsersInsert.events({
	"submit"                   : function (e, t) {
		...
		validateForm(
			...
			function (values) {
				var scope = Router.current().params.pubId ? {publisher:Router.current().params.pubId}:undefined;
				Permissions.check("add-user", "user",scope);
					...
					if (e) {
						...
						if (values.level === 'publisher' && values.publisherId)
							Permissions.setRoles(userId, [{
								"role": "publisher:publisher-manager-from-user",
