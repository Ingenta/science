...
Router.map(function () {
    this.route('PlainText', {
        ...
        action: function () {
            var obj = Articles.findOne({doi: this.params.publisherDoi + "/" + this.params.articleDoi.replace(/%2F/g,"/")});
            if(!obj){
                            ...
                            Science.FSE.exists(outputPath, function (result) {
                                if (result) {
                                    ...
                                    var headers = {
                                        ...
                                        'Content-Disposition': "attachment; filename=" + article.articledoi.replace(/\//g,"%2F") + ".pdf",
